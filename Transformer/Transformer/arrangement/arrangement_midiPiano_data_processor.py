...
from DatasetManager.helpers import UNKNOWN_SYMBOL, MASK_SYMBOL
...
class ArrangementMidiPianoDataProcessor(DataProcessor):
    def __init__(self, dataset: ArrangementMidipianoDataset,
                 embedding_dim,
                 reducer_input_dim,
        ...
        self.max_len_sequence_encoder = dataset.max_number_messages_piano
        ...
        if self.flag == 'orchestra':
            ...
            self.num_token_per_tick = [len(dataset.index2symbol_piano)]
            self.num_ticks_per_frame = dataset.max_number_messages_piano
            self.num_frames = 1
        ...
        self.instrument_labels = nn.Parameter(
            ...
            requires_grad=False)
        self.instrument_embedding = nn.Embedding(self.num_ticks_per_frame, local_position_embedding_dim)

        self.name = 'arrangement_midiPiano_data_processor'
        return
    ...
    def preprocessing(self, *tensors):
        ...
        if self.flag == 'orchestra':
            ...
            return orchestra_past_present.long().cuda(non_blocking=True), None
        elif self.flag == 'piano':
            return piano.long().cuda(non_blocking=True), None
        elif self.flag == 'instruments':
    ...
    def embed(self, x):
        ...
        :return: seq: (batch_size, time * num_voices, embedding_size)1
        """
        if self.flag == 'piano':
            #  Deal with the case of single piano frame
            ret = self.note_embeddings[0](x)
        else:
    ...
    def mask_STANDARD(self, x, p=None, epoch_id=None):
    ...
    def mask_instrument_activations(self, x, p=None):
        ...
        mask_flat = cuda_variable(torch.tensor(mask_np)).long()  #  1 means masked
        mask = mask_flat.unsqueeze(1).repeat(1, num_frames, 1)

        pad_matrix = cuda_variable(
            self.dataset.precomputed_vectors_orchestra_instruments_presence[UNKNOWN_SYMBOL].long())
    ...
    def extract_context_for_generation(self, frame_index, context_size, matrix):
        if self.flag == 'piano':
            #  Chunks are directly represented in piano matrix
            #  Indexing is also slightly different in consequence
            chunk_index = frame_index - context_size
            return matrix[:, chunk_index]
        elif self.flag == 'orchestra':
            start_frame = frame_index - context_size
            end_frame = frame_index + context_size
            return matrix[:, start_frame:end_frame + 1, :]
        elif self.flag == 'instruments':
            start_frame = frame_index - context_size
            end_frame = frame_index + context_size
    ...
    def get_range_generation(self, context_size, number_piano_frames_to_orchestrate):
        ...
        last_frame = number_piano_frames_to_orchestrate + context_size
        events = range(first_frame, last_frame)
        return events
