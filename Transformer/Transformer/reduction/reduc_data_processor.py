...
class ReductionDataProcessor(DataProcessor):
    def __init__(self, dataset: ArrangementDataset,
        ...
        super(ReductionDataProcessor, self).__init__(dataset=dataset,
        ...
        self.num_frames_piano = int(
            (dataset.sequence_size + 1) / 4)  # Only past and present is used for the orchestration
        ...
        # Generic names
        self.max_len_sequence_decoder = self.num_pitch_piano * self.num_frames_piano
    ...
    def init_reduction_filepath(self, batch_size, filepath, banned_instruments=[], unknown_instruments=[],
        ...
        quantized_pianoroll_orchestra = {k: quantize_velocity_pianoroll_frame(v, self.dataset.velocity_quantization)
        ...
        return piano_init.long().cuda(), rhythm_orchestra_clean, orchestra_init.long().cuda()
