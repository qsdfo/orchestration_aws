...
#                                    mxType, idFound])
    ...
    def xmlHarmony(self, mxHarmony):
        ...
        j = self.xmlToJazzChord(mxHarmony)
        # self.insertCoreAndRef(self.offsetMeasureNote, mxHarmony, h)
        self.insertCoreAndRef(self.offsetMeasureNote, mxHarmony, j)

    def xmlToJazzChord(self, mxHarmony):
        '''
        Convert a <harmony> tag to a chord_symbols.JazzChord object
        '''
        # get chord root
        ...
        if mxRoot is not None: # choice: <root> or <function>
            ...
            r_pitch = pitch.Pitch(rootText)
            mxRootAlter =  mxRoot.find('root-alter')
            if mxRootAlter is not None:
                # can provide integer or float to create accidental on pitch
                r_pitch.accidental = pitch.Accidental(float(mxRootAlter.text))
        else:
            # create dedault root ntoe
            r_pitch = pitch.Pitch('C')
        ...
        if mxKind is not None and mxKind.text is not None: # two ways of doing it...
            ...
            if mxKindText is not None:
                chord_id = JazzChord.get_chord_id_from_name_str(mxKindText)
            else:
                chord_id = 1
        else:
            # Initialize default Major chord
            chord_id = 1

        # create JazzChord
        jc = JazzChord(chord_id, r_pitch)
        self.setEditorial(mxHarmony, jc)
        self.setPrintStyle(mxHarmony, jc)
        self.setPrintObject(mxHarmony, jc)
